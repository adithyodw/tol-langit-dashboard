...
interface IcProps { n: string; size?: number; color?: string; sx?: React.CSSProperties; }
...
const TIPS: Record<string, string> = {
  totalRet:  'Total Return — Cumulative percentage gain on the live account since inception, independently verified on MQL5 and/or MyFXBook. All figures are drawn from real-money trading on IC Markets live accounts. No simulated, backtested, or demo results are included.',
  ...
  winRate:   'Win Rate — Percentage of closed trades resulting in a net profit. Always evaluate win rate in conjunction with profit factor and average win/loss ratio.',
  balDD:     'Max Balance Drawdown — The largest peak-to-trough percentage decline in realised account balance, measured from closed trades only. Floating losses are excluded.',
  ...
  trades:    'Total Trades — Number of completed trade executions since inception. Higher trade count provides greater statistical confidence.',
  ...
  rorLvl:    'Risk of Ruin Assessment — Institutional-grade evaluation of near-total capital loss probability. Factors: max drawdown, lot sizing, grid mechanics, track record length, and trade count.',
  days:      'Trading Days — Total calendar days since inception on which at least one trade was active.',
  actRate:   'Activity Rate — Percentage of total calendar days on which at least one trade was opened or active.',
};
...
const ROWS = [
  {g:'Strategy Identity',rows:[
    {l:'Ticker',ic:'briefcase',f:(s:Strategy)=><span style={{fontFamily:MONO,fontSize:12,fontWeight:700,color:C.navy}}>{s.ticker}</span>},
    {l:'Risk Classification',ic:'shield',f:(s:Strategy)=><RBadge s={s}/>},
    {l:'Strategy Type',ic:'sliders',f:(s:Strategy)=><VC main={s.stype} sub={s.snote}/>},
    {l:'Markets Traded',ic:'globe',f:(s:Strategy)=><VC main={s.instr} sub={s.instrS}/>},
    {l:'Inception',ic:'calendar',f:(s:Strategy)=><VC main={s.dur} sub={s.durS}/>},
  ]},
  {g:'Performance',rows:[
    {l:'Total Return',ic:'trending_up',tip:TIPS.totalRet,f:(s:Strategy)=><VC main={s.gain} color='pos'/>},
    {l:'Monthly Return',ic:'bar_chart',tip:TIPS.mo,f:(s:Strategy)=><VC main={s.mo} sub={s.ann} color='pos'/>},
    {l:'Win Rate',ic:'target',tip:TIPS.winRate,f:(s:Strategy)=><VC main={s.win} sub={`${s.wins}W / ${s.losses}L of ${s.trades}`} color='pos'/>},
    {l:'Profit Factor',ic:'activity',tip:TIPS.pf,f:(s:Strategy)=><VC main={s.pf} sub='Gross profit ÷ gross loss' color='pos'/>},
    {l:'Trading Days',ic:'calendar',tip:TIPS.days,f:(s:Strategy)=><VC main={s.days} sub={`${s.act} activity rate`}/>},
    {l:'Trades / Week',ic:'zap',f:(s:Strategy)=><VC main={s.tpw} sub={`Last trade: ${s.lastT}`}/>},
  ]},
  {g:'Risk & Drawdown',rows:[
    {l:'Balance Drawdown',ic:'trending_down',tip:TIPS.balDD,f:(s:Strategy)=><VC main={s.ddBal} sub='Realised balance reduction' color={s.ddBalC}/>},
    {l:'Equity Drawdown',ic:'trending_down',tip:TIPS.eqDD,f:(s:Strategy)=><VC main={s.ddEq} sub='Floating unrealised exposure' color={s.ddEqC}/>},
    {l:'Lot / $1,000',ic:'sliders',tip:TIPS.lot,f:(s:Strategy)=><VC main={s.lot} sub={s.lotN}/>},
    {l:'Grid / Martingale',ic:'alert_tri',tip:TIPS.grid,f:(s:Strategy)=>(
      <div><Tag t={s.gridT==='green'?'green':'amber'}>{s.grid}</Tag><div style={{fontSize:10,color:C.label,marginTop:5}}>{s.gridN}</div></div>
    )},
    {l:'Volatility Profile',ic:'activity',f:(s:Strategy)=><VC main={s.volP}/>},
  ]},
  {g:'Execution',rows:[
    {l:'Avg Hold Time',ic:'clock',tip:TIPS.hold,f:(s:Strategy)=><VC main={s.hold} sub={s.holdS}/>},
    {l:'Platform',ic:'building',f:(s:Strategy)=><VC main={`MetaTrader ${s.platform==='MT4'?'4 (MT4)':'5 (MT5)'}`} sub='IC Markets · ASIC Regulated'/>},
  ]},
  {g:'Portfolio Fit',rows:[
    {l:'Allocation Category',ic:'briefcase',f:(s:Strategy)=>(
    ...
    {l:'Risk of Ruin',ic:'shield',tip:TIPS.rorLvl,f:(s:Strategy)=>(
      <div>
...
const Tag = ({children,t='green'}:{children:React.ReactNode;t?:string}) => {
  const m:{[k:string]:{bg:string;c:string}} = {
    green:{bg:C.posBg,c:C.positive},red:{bg:C.negBg,c:C.negative},
    amber:{bg:C.ambBg,c:C.amber},blue:{bg:C.blueBg,c:C.blue},gray:{bg:C.offWhite,c:C.muted},
  };
  const s = m[t] ?? m.green;
...
const SL = ({c}:{c:string}) => <div style={{fontSize:10,letterSpacing:2.5,textTransform:'uppercase',color:C.gold,marginBottom:8,fontWeight:500}}>{c}</div>;
const ST = ({c,light}:{c:string;light?:boolean}) => <div style={{fontFamily:SERIF,fontSize:'clamp(22px,3vw,28px)',fontWeight:500,color:light?C.white:C.navy,lineHeight:1.2}}>{c}</div>;
const SN = ({c,light}:{c:string;light?:boolean}) => <div style={{fontSize:12,color:light?'rgba(255,255,255,.4)':C.label,textAlign:'right',maxWidth:280,lineHeight:1.55}} className="sh-note">{c}</div>;

const VC = ({main,sub,color='default'}:{main:string;sub?:string;color?:string}) => {
  const cm:{[k:string]:string} = {pos:C.positive,neg:C.negative,amber:C.amber,blue:C.blue,default:C.navy,gray:C.muted};
  return (
    <div>
...
const IRH = ({icon,label,iconC=C.gold,children}:{icon:string;label:React.ReactNode;iconC?:string;children:React.ReactNode}) => (
...
const Tooltip = ({tip}:{tip:string}) => {
  ...
  useEffect(() => {
    if (!vis) return;
    const close = (e: Event) => { if (ref.current && !ref.current.contains(e.target as Node)) setVis(false); };
  ...
  return (
    <div className="tl-hero" style={{background:C.navy,backgroundImage:`linear-gradient(to bottom right,#001a4a 0%,${C.navy} 60%,#000c20 100%)`,padding:'90px 40px 80px',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{maxWidth:1280,margin:'0 auto',position:'relative',zIndex:1}}>
        ...
        {loading && (
          <div style={{marginTop:16,fontSize:11,color:'rgba(255,255,255,.3)',letterSpacing:.5,display:'flex',alignItems:'center',gap:8}}>
      ...
      <div className="tl-2col">
        <div>
          <div style={{border:`1px solid ${C.rule}`,padding:28,textAlign:'center'}}>
            <div style={{width:88,height:88,borderRadius:'50%',background:C.navy,margin:'0 auto 16px',border:`2px solid ${C.rule}`,overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
              ...
              <img src={MANAGER.avatar} alt={MANAGER.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
            </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8,borderTop:`1px solid ${C.rule}`,paddingTop:16,textAlign:'left'}}>
              {([['map_pin',MANAGER.location],['briefcase',MANAGER.employer],['clock',`Active Since ${MANAGER.activeSince}`]] as [string,string][]).map(([ic,t],i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',gap:8,fontSize:12,color:C.muted}}>
                  <Ic n={ic} size={12} color={C.gold}/>{t}
                </div>
              ))}
            </div>
        ...
        <div>
          ...
          <div className="tl-facts" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:1,background:C.rule,border:`1px solid ${C.rule}`,marginTop:28}}>
            {([{n:'5+',l:'Years Live Track Record',g:true},{n:'4,519',l:'Total Trades (V10 Flagship)'},{n:'77–94%',l:'Win Rates Across Signals',g:true}]).map((f,i)=>(
            ...
            <div style={{background:C.offWhite,padding:'24px 28px'}}>
              ...
              <div style={{marginTop:16,borderTop:`1px solid ${C.rule}`,paddingTop:16}}>
                <div style={{fontSize:10,letterSpacing:1.5,textTransform:'uppercase',color:C.label,marginBottom:8,display:'flex',alignItems:'center',gap:6}}>
                  <Ic n="alert_tri" size={11} color={C.label}/>Risk of Ruin Assessment<Tooltip tip={TIPS.rorLvl}/>
                </div>
...
const CT_PLATFORMS:{[k:string]:{name:string;shortName:string;icon:string;desc:string}} = {
  mql5:{name:'MQL5 Signals',shortName:'MQL5',icon:'activity',desc:'Subscribe directly via MetaTrader\'s built-in signal service. Trades are replicated automatically into your MT4/MT5 account in real time. Industry-standard copy mechanism trusted by over one million traders worldwide. No third-party software required.'},
  ss:{name:'SignalStart',shortName:'SignalStart',icon:'zap',desc:'Independent signal monitoring and copy-trading platform. Provides advanced risk controls including equity stop-loss and maximum drawdown thresholds. IC Markets certified provider. Detailed performance analytics with live account verification.'},
  zulu:{name:'ZuluTrade',shortName:'ZuluTrade',icon:'users',desc:'Global social and copy trading platform with millions of registered users. Regulated across multiple jurisdictions. Supports advanced performance filtering, risk scoring per signal provider, and customisable lot-size scaling relative to your account balance.'},
};
...
const CopyTrading = ({S}:{S:Strategy[]}) => {
  ...
  ] as ({key:string;url:string;name:string;shortName:string;icon:string;desc:string}|false)[]).filter(Boolean) as {key:string;url:string;name:string;shortName:string;icon:string;desc:string}[];
    ...
    <div className="sp" style={{padding:'72px 40px',background:C.navy}} id="copy-trading">
      <div style={{maxWidth:1280,margin:'0 auto'}}>
        ...
        <div className="tl-tab-wrap" style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:28}}>
          {S.map((st,i)=>(
            <button key={st.id} className={`tl-tab${act===i?' on':''}`}
              ...
              {platforms.map(pl=>(
                <div key={pl.key} className="tl-ct-card">
                  <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:20}}>
                    <div style={{width:36,height:36,background:'rgba(184,154,62,.12)',border:`1px solid rgba(184,154,62,.2)`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
    ...
    <div style={{maxWidth:1280,margin:'0 auto'}}>
      <div className="sh-flex" style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',flexWrap:'wrap',gap:16,marginBottom:28}}>
        ...
        <div style={{textAlign:'right'}}>
          ...
          <div style={{fontFamily:MONO,fontSize:'clamp(20px,3vw,28px)',fontWeight:700,color:C.goldBright}}>~67.1% p.a.</div>
        </div>
      </div>
      ...
      <div style={{marginTop:20,display:'flex',alignItems:'center',gap:24,flexWrap:'wrap'}}>
        <div style={{fontSize:11,color:'rgba(255,255,255,.35)'}}>Cumulative Return Since Inception: <span style={{fontFamily:MONO,color:C.goldBright,fontWeight:600}}>+1,447.47%</span></div>
  ...
  <div className="sp" style={{padding:'56px 40px',background:C.offWhite,borderTop:`3px solid ${C.negative}`}} id="disclosures">
    <div style={{maxWidth:1280,margin:'0 auto'}}>
      ...
      <div className="tl-2col" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:1,background:C.rule,border:`1px solid ${C.rule}`,marginBottom:1}}>
        ...
        ] as {ic:string;t:string;b:string;w:string|null}[]).map((b,i)=>(
...
export default function TolLangitDashboard() {
  ...
  useEffect(() => {
    ...
    return () => { if (dateTimerRef.current) clearInterval(dateTimerRef.current); };
  }, []);
  ...
  return (
    <div style={{ fontFamily: SANS, fontSize: 14, color: C.body, background: C.white }}>
      ...
      <div className="tl-float">
        {msg && (
          <div style={{ background: C.navy, color: C.goldBright, padding: '6px 14px', fontSize: 10, fontFamily: MONO, letterSpacing: .5, border: `1px solid ${C.gold}`, opacity: .92, maxWidth: 'calc(100vw - 48px)', wordBreak: 'break-word' }}>
            {msg}
          </div>
        )}
