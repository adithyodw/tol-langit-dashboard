...
const BENCH_ASSETS = [
  ...
  { key:'gold',   label:'Gold (XAU)',     short:'Gold',   color:'#FFD700', group:'bench', note:'Spot XAU/USD price return' },
  { key:'silver', label:'Silver (XAG)',   short:'Silver', color:'#b0c4d8', group:'bench', note:'Spot XAG/USD price return' },
  { key:'nvda',   label:'NVIDIA (NVDA)', short:'NVDA',   color:'#76b900', group:'bench', note:'NVDA split-adjusted price return' },
  { key:'tsla',   label:'Tesla (TSLA)',  short:'Tesla',  color:'#e31937', group:'bench', note:'TSLA split-adjusted price return' },
  { key:'ndx',    label:'Nasdaq 100',    short:'NDX',    color:'#818cf8', group:'bench', note:'NDX price return (no dividends)' },
  { key:'spx',    label:'S&P 500',       short:'SPX',    color:'#a78bfa', group:'bench', note:'SPX price return (no dividends)' },
];
const ALL_ASSETS = [...TL_ASSETS, ...BENCH_ASSETS];
const AMOUNTS = [1000, 5000, 10000, 25000, 50000, 100000];

/* ─── Period presets ─────────────────────────────────────────────────────── */
// idx: 0=Jan2021, 63=Apr2026. YTD=Jan2026(60). 1Y=Apr2025(51). 2Y=Apr2024(39). 3Y=Apr2023(27). 5Y=Apr2021(3).
const PRESETS = [
  { label: 'YTD', start: 60, end: 63 },
  { label: '1Y',  start: 51, end: 63 },
  { label: '2Y',  start: 39, end: 63 },
  { label: '3Y',  start: 27, end: 63 },
  { label: '5Y',  start:  3, end: 63 },
  { label: 'MAX', start:  0, end: 63 },
];
...
export default function SimulationSection() {
    ...
    for (let i = startIdx; i <= endIdx; i++) {
      ...
      for (const key of active) {
        ...
        if (raw !== null && raw !== undefined) {
          ...
          if (baseRaw) { row[key] = Math.round((raw / baseRaw) * amount); hasValue = true; }
        }
      }
  ...
  const btnBase: React.CSSProperties = {
    padding: '8px 14px', fontFamily: MONO, fontSize: 11, cursor: 'pointer',
    borderRadius: 0, minHeight: 38, transition: 'all .15s', fontWeight: 600,
  };
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div className="sh-flex" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 32, paddingBottom: 20, borderBottom: `1px solid ${C.ruleDark}` }}>
          <div>
            ...
            <p style={{ fontSize: 12.5, color: 'rgba(255,255,255,.35)', marginTop: 6, lineHeight: 1.5 }}>Select a strategy and compare against global benchmarks across any time period.</p>
          </div>
        ...
        {/* ── Controls ── */}
        <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', marginBottom: 24, alignItems: 'flex-start' }}>

          {/* Amount */}
          <div>
            <div style={{ fontSize: 9, letterSpacing: 2, textTransform: 'uppercase', color: 'rgba(255,255,255,.3)', marginBottom: 8, fontFamily: MONO }}>Initial Investment</div>
          ...
          {/* Period */}
          <div>
            <div style={{ fontSize: 9, letterSpacing: 2, textTransform: 'uppercase', color: 'rgba(255,255,255,.3)', marginBottom: 8, fontFamily: MONO }}>Period</div>
        ...
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,.2)', fontFamily: MONO, marginBottom: 20, paddingLeft: 4 }}>
          {ALL_MONTHS[startIdx]} → {ALL_MONTHS[endIdx]} · ${amount.toLocaleString()} invested at period start, normalized per asset inception
        </div>
        ...
        <div style={{ marginTop: 16, padding: '12px 16px', border: `1px solid rgba(184,154,62,.12)`, background: 'rgba(184,154,62,.03)' }}>
          <p style={{ fontSize: 10.5, color: 'rgba(255,255,255,.25)', lineHeight: 1.75, fontFamily: SANS, margin: 0 }}>
            ...
            All TL data from live MQL5 & MyFXBook verified accounts. Benchmark figures are approximate spot-price returns and exclude dividends, fees, and taxes.
