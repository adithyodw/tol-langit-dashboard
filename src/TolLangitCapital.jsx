...
const BASE = [
  {
    ...
    platform:"MT4",
    /* Monthly return updated from latest MQL5 statistics. */
    gain:"+1,407.55%",win:"81.45%",pf:"2.71",mo:"+0.60%",ann:"~67.6% p.a. (5-yr CAGR)",
    ...
    tpw:"0",lastT:"22 days ago",hold:"~3-7 Days",holdS:"Multi-Day Swing / Positional",
  ...
  },
  {
    ...
    platform:"MT4",
    gain:"+900.23%",win:"94.09%",pf:"2.42",mo:"+4.62%",ann:"~228.77% p.a.",
    ddBal:"17.74%",ddBalC:"neg",ddEq:"90.58%",ddEqC:"neg",
    trades:"711",wins:"669",losses:"42",days:"170",act:"94.15%",
    lot:"0.11–0.20",lotN:"Aggressive",dur:"~12 Months",durS:"50 weeks · Inception Mar 2025",
    instr:"11 Forex Pairs",instrS:"EUR/USD, GBP/USD, USD/JPY + 8 cross pairs",
    tpw:"0",lastT:"9 days ago",hold:"~3-5 Days",holdS:"Multi-Day Swing / Positional",
  ...
  },
  {
    ...
    platform:"MT5",
    gain:"+193.45%",win:"86.25%",pf:"4.10",mo:"+40.79%",ann:"Insufficient history",
    ddBal:"8.38%",ddBalC:"amber",ddEq:"62.36%",ddEqC:"amber",
    trades:"211",wins:"182",losses:"29",days:"26",act:"64.96%",
    ...
    tpw:"10",lastT:"8 hours ago",hold:"~19 Hours",holdS:"Near-Intraday",
  ...
  },
  {
    ...
    myfx:null,
    platform:"MT5",
    gain:"+37.53%",win:"100%",pf:"73.22",mo:"+37.53%",ann:"Insufficient history",
    ddBal:"0.14%",ddBalC:"pos",ddEq:"32.45%",ddEqC:"amber",
    trades:"20",wins:"20",losses:"0",days:"3",act:"0.40%",
    ...
    tpw:"6",lastT:"8 hours ago",hold:"~8 Minutes",holdS:"High-Frequency Scalping",
    ...
    <div className="tl-hero" style={{background:C.navy,backgroundImage:`linear-gradient(to bottom right,#001a4a 0%,${C.navy} 60%,#000c20 100%)`,padding:"90px 40px 80px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:1280,margin:"0 auto",position:"relative",zIndex:1}}>
        ...
        {loading&&<div style={{marginTop:16,fontSize:11,color:"rgba(255,255,255,.3)",letterSpacing:.5,display:"flex",alignItems:"center",gap:8}}>
          ...
          Retrieving live performance data from Myfxbook…
        </div>}
      </div>
    </div>
  );
  ...
  <div className="sp" style={{padding:"72px 40px",background:C.offWhite}} id="comparison">
    <div style={{maxWidth:1280,margin:"0 auto"}}>
      <div className="sh-flex" style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:32,paddingBottom:20,borderBottom:`1px solid ${C.rule}`}}>
        ...
        <SN c="All metrics sourced from verified live accounts. Data auto-refreshed from the Myfxbook API at session load and every 15 minutes."/>
      </div>
...
export default function App() {
  ...
  const fetchLive = useCallback(async () => {
    ...
    try {
      ...
      // If backend returns null values, keep existing BASE data
      if (!p || (!p.v10 && !p.v10hr && !p.etf && !p.etfgold)) {
        setApiDate(null);
        setMsg("Using verified cached data");
      } else {
        const merged = BASE.map((s, idx) => {
          const key = ["v10", "v10hr", "etf", "etfgold"][idx];
          const lv = p[key];
          if (!lv) return s;
          return { ...s, ...Object.fromEntries(Object.entries(lv).filter(([, v]) => v != null)) };
        });
        setS(merged);
        const ts = p.updatedAt || getDeviceDate();
        setApiDate(ts);
        setMsg(`Data refreshed — ${ts} · Myfxbook`);
      }
    } catch (e) {
